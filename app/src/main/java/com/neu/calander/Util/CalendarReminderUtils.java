...
public class CalendarReminderUtils {
     ...
     */
    @RequiresApi(api = Build.VERSION_CODES.N)
    public static void addCalendarEvent(Context context, String title, String description, long reminderTime, int previousDate, Data_List data_list) throws ParseException {
        if (context == null) {
            return;
        }
        ...
        event.put("title", title+data_list.getId());
        ...
        if(data_list.isRepeat()) {
            if (data_list.getRepeat_flag().equals("周")) {
                ...
                event.put(CalendarContract.Events.RRULE, "FREQ=WEEKLY;BYDAY=" + week[DateUtil.get_week_of_date_int(sdf.parse(data_list.get_format()))]);
            }
            if (data_list.getRepeat_flag().equals("月")) {
                event.put(CalendarContract.Events.RRULE, "FREQ=MONTHLY;BYMONTHDAY=" + data_list.getDay());
            }
            if (data_list.getRepeat_flag().equals("天")) {
     ...
     * 删除日历事件
     */
    public static void deleteCalendarEvent(Context context, String title,Data_List data_list) {
        if (context == null) {
            return;
        }
